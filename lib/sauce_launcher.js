...
function processConfig (helper, config, args) {
  ...
  /* if a parent tunnel has been specified, it leads the choice */
  var parentTunnel = config.parentTunnel || process.env.SAUCE_PARENT_TUNNEL
  var startConnect = (!parentTunnel) ? config.startConnect !== false : true
  ...
  var browserName = args.browserName + (args.version ? ' ' + args.version : '') + (args.platform ? ' (' + args.platform + ')' : '')
...
var SauceLauncher = function (
  ...
  var username = connectOptions['username']
  var accessKey = connectOptions['accessKey']
  ...
  self.on('start', function (url) {
    ...
    if (startConnect && !options.parentTunnel && options.tunnelIdentifier) {
